...
def replace_ids_with_levels(input_file, output_file, src_mapping, dst_mapping, offset=6, knowledge_mapping=None):
    """
    ...
    with open(output_file, 'w') as f:
        for line in lines:
            try:
                ...
                if knowledge_mapping and src in knowledge_mapping:
                    new_src += offset
                if knowledge_mapping and dst in knowledge_mapping:
                    new_dst += offset
...
if __name__ == '__main__':
    # 输入文件路径
    question_file = './data/Read/question_one_hot_encoded.csv'
    knowledge_file = './data/Read/knowledge_one_hot_encoded.csv'

    # 加载映射关系
    question_mapping, knowledge_mapping = load_mappings(question_file, knowledge_file)

    # 处理并替换文件内容
    replace_ids_with_levels(
        './data/Read/graph/k_from_e.txt',
        './data/Read/graph/k_from_e_mapped.txt',
        question_mapping,
        knowledge_mapping,
        offset=6,
        knowledge_mapping=knowledge_mapping  # 传入知识点映射
    )
    replace_ids_with_levels(
        './data/Read/graph/e_from_k.txt',
        './data/Read/graph/e_from_k_mapped.txt',
        knowledge_mapping,
        question_mapping,
        offset=6,
        knowledge_mapping=knowledge_mapping  # 传入知识点映射
